...
export function candidateIdempotencyKey(sourceTopicId, version, type) {
    return `toutiao:${sourceTopicId}:${version}:${type}`;
}
/** `{sourceEventId}:write:v1` */
export function writingIdempotencyKey(sourceEventId) {
 ...
 */
export function validateIdempotencyKeyFormat(key) {
