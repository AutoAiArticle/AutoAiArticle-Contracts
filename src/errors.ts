...
export const SIGNATURE_ERROR_STATUS: Record<string, number> = {
  unknown_key_id: 401,
 ...
 */
export function classifyHttpStatus(httpStatus: number | null): RetryClass {
  if (httpStatus === null || (httpStatus >= 500 && httpStatus <= 599)) {
    return 'network_or_5xx';
  }
  if (httpStatus === 429) return 'rate_limited';
  if ([400, 401, 403, 404].includes(httpStatus)) return 'auth_or_config';
  return 'final';
}

const BACKOFF_SECONDS: Record<Exclude<RetryClass, 'final'>, number[]> = {
